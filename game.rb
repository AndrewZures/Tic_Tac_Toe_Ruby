class Game
    ...
    def run_game_loop player_string, move
        player = get_player_object(player_string)
        if player == @current_player
            if player.type == :human
                result = @board.record_choice(move, player.symbol)
    ...
    def correctly_set_up? 
        if @id != null && @board != null && @player1 != null && @player2 != null then return true
        else return false
        end
    end
    ...
    def make_ai_moves
        while @current_player.type != :human
            if(board.game_over?) then break end
